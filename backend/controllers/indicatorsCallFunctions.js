...
const getMPMI = async (request, response) => {
  ...
  if (!ISMManufacturing.data.series) {
...
const getSPMI = async (request, response) => {
  ...
  if (!ISMNonManufacturing.data.series) {
    return response.status(403).json({
      error: "can't access DB.NOMICS api",
    });
  }
...
const getMichigan = async (request, response) => {
  ...
  if (!michiganSentiment.data.series) {
    return response.status(403).json({
      error: "can't access DB.NOMICS api",
    });
  }
...
const getTreasury10Yield = async (request, response) => {
  ...
  if (bondYield.data.Note === vantageNote) {
    return response.status(429).json({
      error:
        "can't get treasury yield data, too many requests, reached Vantage api limit (500/day 5/minute)",
    });
  }

  if (!bondYield.data.data) {
    return response.status(403).json({
...
const getGDP = async (request, response) => {
  ...
  if (gdp.data.Note === vantageNote) {
    return response.status(429).json({
      error:
        "can't get gdp data, too many requests, reached Vantage api limit (500/day 5/minute)",
    });
  }
...
const getPayrolls = async (request, response) => {
  ...
  if (payroll.data.Note === vantageNote) {
    return response.status(429).json({
      error:
        "can't get nonfarm payroll data, too many requests, reached Vantage api limit (500/day 5/minute)",
    });
  }
